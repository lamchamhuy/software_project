...
public function redirectToMomo(Order $order)
{
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'payment_method' => 'required|in:cod,bank_transfer,qr_payment,momo,vnpay,stripe',
    ...
    private function handlePayment(Order $order, string $paymentMethod): RedirectResponse
    {
        switch ($paymentMethod) {
            ...
            case 'qr_payment':
                return redirect()->route('checkout.qr-payment', ['orderId' => $order->order_id]);
            case 'momo':
    ...
    public function qrPayment($orderId): View
    {
        $order = Order::where('order_id', $orderId)
            ->where('user_id', Auth::id())
            ->firstOrFail();
        ...
        $bank = [
            'code' => env('QR_BANK_CODE', 'VCB'),
            'name' => env('QR_BANK_NAME', 'Vietcombank'),
            'account_number' => env('QR_BANK_ACCOUNT_NUMBER', '1234567890'),
            'account_name' => env('QR_BANK_ACCOUNT_NAME', 'TECHMART COMPANY'),
            'branch' => env('QR_BANK_BRANCH', 'Ha Noi'),
        ];

        $qrImageUrl = 'https://img.vietqr.io/image/' . rawurlencode($bank['code'])
            . '-' . rawurlencode($bank['account_number'])
            . '-compact.png?amount=' . (int) $order->total_amount
            . '&addInfo=' . rawurlencode($order->order_number)
            . '&accountName=' . rawurlencode($bank['account_name']);

        return view('checkout.qr-payment', compact('order', 'bank', 'qrImageUrl'));
    }
