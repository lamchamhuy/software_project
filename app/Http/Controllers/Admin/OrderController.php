...
class OrderController extends Controller
{
    ...
    {
        $query = Order::with(['user', 'shipper', 'orderItems.product', 'orderItems.productVariant']);
        ...
        $shippers = User::where('role', 'shipper')->orderBy('name')->get();
    ...
    public function assignShipper(Request $request, $id)
    {
        ...
        if ($shipperId && ! User::where('id', $shipperId)->where('role', 'shipper')->exists()) {
            return back()->with('error', 'Nguoi duoc chon khong phai shipper.');
        }

        $order = Order::where((new Order())->getKeyName(), $id)->firstOrFail();

        if (in_array($order->status, ['delivered', 'completed', 'cancelled'], true)) {
            return back()->with('error', 'Khong the phan cong don da ket thuc.');
        }

        $order->shipper_id = $shipperId ?: null;

        if ($shipperId && in_array($order->status, ['pending', 'delivery_failed'], true)) {
            $order->status = 'processing';
        }

        if (! $shipperId && $order->status === 'shipped') {
            $order->status = 'processing';
        ...
        return back()->with('success', $shipperId ? 'Da phan cong nguoi giao hang.' : 'Da bo phan cong nguoi giao hang.');
    }
    ...
    public function updateStatus(Request $request, $id)
    {
        ...
        $request->validate([
            'status' => 'required|in:pending,processing,shipped,delivered,completed,delivery_failed,cancelled'
        ]);
        ...
        try {
            ...
            if ($newStatus === 'shipped' && ! $order->shipper_id) {
                return redirect()->back()->with('error', 'Hay phan cong nguoi giao hang truoc khi chuyen don sang trang thai dang giao.');
            }
